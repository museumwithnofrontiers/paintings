...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('paintings.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('paintings.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('paintings.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
